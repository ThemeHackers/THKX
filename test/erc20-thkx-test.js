...
describe("THKXToken", function () {
  ...
  beforeEach(async function () {
    ...
    await thkxToken.waitForDeployment();
  });

  it("Should deploy with correct initial supply", async function () {
    const totalSupply = await thkxToken.totalSupply();
    expect(totalSupply).to.equal(ethers.parseEther("700000000"));
  });

  it("Should allow staking", async function () {
    await thkxToken.transfer(addr1.address, ethers.parseEther("100"));
    await thkxToken.connect(addr1).stake(ethers.parseEther("50"));
    const stakeInfo = await thkxToken.getStakeInfo(addr1.address);
    expect(stakeInfo.stakeAmount).to.equal(ethers.parseEther("50"));
  });
  ...
  it("Should allow unstaking with rewards", async function () {
    ...
    await thkxToken.connect(addr1).unstake(ethers.parseEther("50"));
    const balance = await thkxToken.balanceOf(addr1.address);
    expect(balance).to.be.above(ethers.parseEther("100")); 
  });
  ...
  it("Should allow emergency withdrawal", async function () {
    ...
    const stakeInfo = await thkxToken.getStakeInfo(addr1.address);
    expect(stakeInfo.stakeAmount).to.equal(0n);
  });
  ...
  it("Should correctly set and execute timelocked reward rate update", async function () {
    ...
    expect(await thkxToken.rewardRate()).to.equal(500n);
  });
